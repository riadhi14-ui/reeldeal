...
export default function SettingsPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast({ title: "Réglages enregistrés", description: "Tes préférences ont été mises à jour." });
    } catch (err) {
        ...
        <section>
          <button onClick={() => base44.auth.logout("/")} className="w-full flex items-center gap-3 px-6 py-4 rounded-3xl bg-white ring-1 ring-red-100 text-[#DC2626] font-bold hover:bg-red-50 transition-colors">
            ...
            <div className="text-left">
              <p>Se déconnecter</p>
              <p className="text-xs font-normal text-red-400">Se déconnecter de ton compte</p>
            </div>
          </button>
        </section>
