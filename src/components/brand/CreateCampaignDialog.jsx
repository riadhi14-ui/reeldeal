...
const EMPTY = { name: "", brand: "", category: "", rate: "", budget: "", maxPerVideo: "", description: "", dos: "", donts: "", img: "", explainerVideo: "", exampleVideos: [] };
...
export default function CreateCampaignDialog({ onCreated, campaign = null, open: controlledOpen, onOpenChange }) {
  ...
  useEffect(() => {
    ...
    setForm(campaign ? {
      ...
      dos: (campaign.do_list || campaign.brief || []).join("\n"), donts: (campaign.dont_list || []).join("\n"), img: campaign.img || "", explainerVideo: campaign.explainer_video || "", exampleVideos: campaign.example_videos || [],
    } : EMPTY);
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-3xl rounded-3xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={(event) => { event.preventDefault(); save(campaign?.status === "active" ? "active" : "pending"); }} className="space-y-5">
          ...
          <CampaignMediaFields image={form.img} videos={form.exampleVideos} explainer={form.explainerVideo} onImage={(img) => setForm((current) => ({ ...current, img }))} onVideos={(exampleVideos) => setForm((current) => ({ ...current, exampleVideos }))} onExplainer={(explainerVideo) => setForm((current) => ({ ...current, explainerVideo }))} onUploading={setUploading} onError={setError} />
